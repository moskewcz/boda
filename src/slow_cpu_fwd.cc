...
namespace boda 
{
  ...
  void run_conv_op_one_img_conv( p_conv_op_t const & cop, p_map_str_p_nda_float_t const & fwd, uint32_t const img_ix, 
    ...
    assert_st( filts->dims == dims_t(vect_uint32_t{top->dims.dims(1),bot->dims.dims(1),kern_sz.d[1],kern_sz.d[0] }, "float") );
    assert_st( biases->dims == dims_t(vect_uint32_t{top->dims.dims(1)}, "float") );
  ...
  void run_conv_op_one_img_pool( p_conv_op_t const & cop, p_map_str_p_nda_float_t const & fwd, uint32_t const img_ix, 
    ...
    uint32_t const avg_pool = cop->get_u32("avg_pool");
  ...
  void run_conv_op( p_conv_op_t const & cop, p_map_str_p_nda_float_t const & fwd ) { 
	...
	for( uint32_t y = 0; y != top->dims.dims(2); ++y ) {
      ...
      assert( bot->dims == top->dims );
      assert( bot->dims.sz() == 4 );
      uint32_t const local_size = cop->get_u32("local_size");
      double const alpha = cop->get_double("alpha");
      double const beta = cop->get_double("beta");
      double const k = cop->get_double("k");
      assert( local_size & 1 );
