...
def add_nin_imagenet_jobs( jobs ):
    ...
    qopts_base += ')'
    for i in range(16):
        job = 'boda test_lmdb --model-name=nin_imagenet --num-to-read=50000 --run-cnet="(in_sz=227 227,in_num_imgs=20,ptt_fn=%(models_dir)/%(model_name)/train_val.prototxt,trained_fn=%(models_dir)/%(model_name)/best.caffemodel,out_layer_name=pool4,compute_mode=1,conv_fwd=(mode=nvrtc,enable_stats=0,show_rtc_calls=0,'+(qopts_base%{'bits':i})+'))"'
        jobs.append( job )
