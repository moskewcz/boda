...
class GenObjList( object ):
    ...
    def __init__( self ):    
        ...
        for dep_name, dep in self.deps.iteritems():
            ...
            # FIXME: hard-coded hack to track files generated by protoc command (presumably) inside this dep
            if dep_name == "caffe_pb":
                self.gen_fns.add( "caffe.pb.cc" )
                self.gen_fns.add( "caffe.pb.h" )
        dep_make.close();
