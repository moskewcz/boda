...
extern "C"  __global__ void %(cu_func_name)( float const * const in, float * const out ) {
  ...
  uint32_t const hls = %(local_size) >> 1;
  uint32_t const out_base_ix = %(tix_img)*%(out_ix_img_sz) + %(tix_y)*%(out_ix_y_sz) + %(tix_x)*%(out_ix_x_sz);  
  for( uint32_t in_chan_ix = 0; in_chan_ix < %(out_ix_chan_dim) + hls; ++in_chan_ix ) {
    uint32_t const in_off = in_chan_ix*%(out_ix_chan_sz);
    uint32_t const lsb_ix = in_chan_ix %% %(local_size);
    ls_buf[lsb_ix] = (in_chan_ix < %(out_ix_chan_dim)) ? in[out_base_ix + in_off] : 0.0f;
    
    if( in_chan_ix >= hls ) {
      uint32_t const out_chan_ix = in_chan_ix - hls;
      float ls_sum = 0.0f;
      for( uint32_t i = 0; i != %(local_size); ++i ) { ls_sum += ls_buf[i]*ls_buf[i]; }
